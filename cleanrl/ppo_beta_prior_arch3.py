...
def capped_quad_video_scheduler(episode_id: int) -> bool:
    if episode_id < 1000:
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        if capture_video and idx == 0:
            ...
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}", capped_quad_video_scheduler)
        else:
            env = gym.make(env_id)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, alpha_beta_max=5):
        action_alpha = alpha_beta_max * self.actor_alpha(x) + 1
        action_beta = alpha_beta_max * self.actor_beta(x) + 1
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    if info and "episode" in info:
                        print(f"global_step={global_step}, episodic_return={info['episode']['r']}, ep_num={ep_num}")
                        ep_num += 1
